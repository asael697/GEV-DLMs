...
alpha = rnorm(1)
# datos
y = gev_rng(n = 250,mu = mu,sigma = sigma,xi = alpha)
...
loglik <- function(y,theta){
    z = (y - theta[1])/theta[2]
    d = gev_lpdf(y,mu = 0, sigma = 1, k = theta[3])
    return(sum(d))
}

# Calculo de la log prior logP(theta)
log_prior <- function(theta){
  d1 = dnorm(theta[1],mean = 0,sd = 0.5,log = TRUE)
  d2 = dgamma(theta[2],shape = 5,rate = 2.5,log = TRUE)
  d3 = dnorm(theta[3],mean = 0,sd = 0.5,log = TRUE)
  return(d1+d2+d3)
}

# Generacion de los valores iniciales
inits <- function( ){
  c(rnorm(1,sd = 0.5),rgamma(1,5,2.5),rnorm(1,sd = 0.5))
}
```
...
```{r,warning=FALSE}
scale_mat =  diag(c(0.5,.5,.4))

post1 = metropolis_sampler(y = y, scale = scale_mat, chains = 4, iter = 5000,
                           thin = 5, dif_adjust = TRUE)
```
